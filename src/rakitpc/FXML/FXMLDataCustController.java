...
public class FXMLDataCustController implements Initializable {
    ...
    // Metode untuk menampilkan kolom di tabel
    ...
    public void showdata() {
        ...
        TableColumn<CustModel, Double> col4 = new TableColumn<>("totalbiayarakit");
        col4.setCellValueFactory(new PropertyValueFactory<>("totalbiayarakit"));

        TableColumn<CustModel, String> col5 = new TableColumn<>("status");
        col5.setCellValueFactory(new PropertyValueFactory<>("status"));

        // Menambahkan semua kolom sekaligus ke tabel
        tbvcust.getColumns().addAll(col1, col2, col3, col4, col5);

        if (data == null || data.isEmpty()) {
            // Menambahkan placeholder jika data kosong
    ...
    private void insertclick(ActionEvent event) {
        try {
            ...
            a.showAndWait();
            e.printStackTrace(); // Debugging jika diperlukan
        }
        showdata();
        firstclick(event);
    }
